...
extension String {
    ...
    var colonSaparatedComponents: [String] {
        return self.components(separatedBy: ":")
    }
    var url: URL? {
...
extension Utils{
    ...
    static func checkEnglishPhoneNumberFormat(string: String?, str: String?,textField:UITextField){
        if string == ""{ //BackSpace
        }else if str!.count < 3{
            if str!.count == 1{
                textField.text = ""
    ...
    class func convertDateFormat2(_ dateString: String, sourceFormate: String, targetFormate: String) -> String{
        if stringExists(dateString){
            //date formatter for the above string
            let dateFormatterWS: DateFormatter = DateFormatter()
            dateFormatterWS.dateFormat = sourceFormate
            ...
            //date formatter that you want
            let dateFormatterNew: DateFormatter = DateFormatter()
            dateFormatterNew.dateFormat = "MM-dd-yyyy HH:mm:ss"
    ...
    // checks whether string value exists or it contains null or null in string
       class func stringExists(_ str: String) -> Bool {
    ...
    /// Trim for String
       class func Trim(_ value: String) -> String {
    ...
    class func getURLFromString(_ str: String) -> String {
        let urlStr: String = str.replacingOccurrences(of: " ", with: "%20")
        return urlStr
    }
    
    static func decodeUnicode(_ str: String) -> String{
       let removeSlashN = str.replacingOccurrences(of: "\\n", with: "\n")
        let data = removeSlashN.data(using: String.Encoding.utf8, allowLossyConversion: false)
        if data != nil{
            let valueunicode = NSString(data: data!, encoding: String.Encoding.nonLossyASCII.rawValue)
            if valueunicode != nil{
                return valueunicode! as String
            }
        }
        return str
    }
...
//    }
    ...
    func offsetFrom(_ date:Date) -> String {
     ...
     func timeAgo() -> String {
        ...
        formatter.allowedUnits = [.year, .month, .day, .hour, .minute, .second,.weekday]
